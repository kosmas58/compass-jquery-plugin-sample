...
t;var A=f.rowNum,B=g.scrollTop=g.bDiv.scrollTop,M=Math.round(s.position().top)-B,I=M+s.height();t=t*A;var R,O,F;if(M<=0&&I<p&&(f.lastpage==null||parseInt((I+B+t-1)/t,10)<f.lastpage)){O=parseInt((p-I+t-1)/t,10);if(I>=0||O<2||f.scroll===true){R=Math.round((I+B)/t)+1;M=-1}else M=1}if(M>0){R=parseInt(B/t,10)+1;O=parseInt((B+p)/t,10)+2-R;F=true}if(O)if(!(f.lastpage&&R>f.lastpage))if(g.hDiv.loading)g.timer=setTimeout(g.populateVisible,200);else{f.page=R;if(F){g.selectionPreserver(s[0]);g.emptyRows(g.bDiv)}g.populate(O)}}}},
...
0}},x=function(p,s){var t,A,B;if(typeof s==="function")return s(p);t=p[s];if(t===undefined){if(typeof s==="string")B=s.split(".");try{if(B.length)for(t=p;t&&B.length;){A=B.shift();t=t[A]}}catch(M){}}return t},N=function(p,s,t,A,B){var M=new Date;c.p.reccount=0;if(a.isXMLDoc(p)){if(c.p.treeANode===-1&&!c.p.scroll){H(s);t=0}else t=t>0?t:0;var I,R=0,O,F,L=0,X=0,ca=0,ha,pa,na=[],ra,ja={},Da=c.rows.length,ia,Aa,fa=[],ua=0,Ea=c.p.altRows===true?" "+c.p.altclass:"";c.p.xmlReader.repeatitems||(na=C("xml"));
...
Aa=pa(ia,Ja+qa);I=t==0?0:t+1;I=(I+qa)%2==1?Ea:"";fa[ua++]='<tr id="'+Aa+'" role="row" class ="ui-widget-content jqgrow ui-row-'+c.p.direction+""+I+'">';if(c.p.rownumbers===true){fa[ua++]=D(0,qa,c.p.page,c.p.rowNum);ca=1}if(c.p.multiselect===true){fa[ua++]=z(Aa,ca,qa);L=1}if(c.p.subGrid===true){fa[ua++]=a(c).jqGrid("addSubGridCell",L+ca,qa+t);X=1}if(c.p.xmlReader.repeatitems){ra||(ra=y(L+X+ca));var Ka=a(c.p.xmlReader.cell,ia);a.each(ra,function(Ba){var ya=Ka[this];if(!ya)return false;O=ya.textContent||
ya.text;ja[c.p.colModel[Ba+L+X+ca].name]=O;fa[ua++]=u(Aa,O,Ba+L+X+ca,qa+t,ia)})}else for(I=0;I<na.length;I++){O=a(na[I],ia).text();ja[c.p.colModel[I+L+X+ca].name]=O;fa[ua++]=u(Aa,O,I+L+X+ca,qa+t,ia)}fa[ua++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){I=c.p.treeANode>=-1?c.p.treeANode:0;F=a(fa.join(""))[0];try{a(c).jqGrid("setTreeNode",ja,F)}catch(La){}Da===0?a("tbody:first",s).append(F):a(c.rows[qa+I+t]).after(F)}else a("tbody:first",s).append(fa.join(""));if(c.p.subGrid===true)try{a(c).jqGrid("addSubGrid",
c.rows[c.rows.length-1],L+ca)}catch(Ma){}B&&c.p.afterInsertRow.call(c,Aa,ja,ia);fa=[];ua=0}ja={};R++;qa++;if(R==Ha)break}}c.p.gridview===true&&a("tbody:first",s).append(fa.join(""));c.p.totaltime=new Date-M;if(R>0){c.grid.cols=c.rows[0].cells;if(c.p.records===0)c.p.records=Fa}fa=null;if(!c.p.treeGrid&&!c.p.scroll)c.grid.bDiv.scrollTop=0;c.p.reccount=R;c.p.treeANode=-1;c.p.userDataOnFooter&&a(c).jqGrid("footerData","set",c.p.userData,true);A||da(false,true)}},Y=function(p,s,t,A,B){var M=new Date;c.p.reccount=
0;if(p){if(c.p.treeANode===-1&&!c.p.scroll){H(s);t=0}else t=t>0?t:0;var I=0,R,O,F,L=[],X,ca=0,ha=0,pa=0,na,ra,ja,Da={},ia,Aa=c.rows.length,fa;F=[];ia=0;var ua=c.p.altRows===true?" "+c.p.altclass:"";c.p.page=x(p,c.p.jsonReader.page)||1;ja=x(p,c.p.jsonReader.total);c.p.lastpage=ja===undefined?1:ja;c.p.records=x(p,c.p.jsonReader.records)||0;c.p.userData=x(p,c.p.jsonReader.userdata)||{};c.p.jsonReader.repeatitems||(X=L=C("json"));ja=c.p.keyIndex===false?c.p.jsonReader.id:c.p.keyIndex;if(L.length>0&&!isNaN(ja)){if(c.p.remapColumns&&
c.p.remapColumns.length)ja=a.inArray(ja,c.p.remapColumns);ja=L[ja]}if(ra=x(p,c.p.jsonReader.root)){na=ra.length;p=0;var Ea=parseInt(c.p.rowNum,10),Fa=c.p.scroll?(parseInt(c.p.page,10)-1)*Ea+1:1;if(B)Ea*=B+1;for(var qa=a.isFunction(c.p.afterInsertRow);p<na;){B=ra[p];fa=B[ja];if(fa===undefined){fa=Fa+p;if(L.length===0)if(c.p.jsonReader.cell)fa=B[c.p.jsonReader.cell][ja]||fa}R=t==0?0:t+1;R=(R+p)%2==1?ua:"";F[ia++]='<tr id="'+fa+'" role="row" class= "ui-widget-content jqgrow ui-row-'+c.p.direction+""+
R+'">';if(c.p.rownumbers===true){F[ia++]=D(0,p,c.p.page,c.p.rowNum);pa=1}if(c.p.multiselect){F[ia++]=z(fa,pa,p);ca=1}if(c.p.subGrid){F[ia++]=a(c).jqGrid("addSubGridCell",ca+pa,p+t);ha=1}if(c.p.jsonReader.repeatitems){if(c.p.jsonReader.cell)B=x(B,c.p.jsonReader.cell);X||(X=y(ca+ha+pa))}for(O=0;O<X.length;O++){R=x(B,X[O]);F[ia++]=u(fa,R,O+ca+ha+pa,p+t,B);Da[c.p.colModel[O+ca+ha+pa].name]=R}F[ia++]="</tr>";if(c.p.gridview===false){if(c.p.treeGrid===true){ia=c.p.treeANode>=-1?c.p.treeANode:0;F=a(F.join(""))[0];
...
s=[],t=0,A,B,M,I=c.p.sortorder=="asc"?1:-1,R=false;a.each(c.p.colModel,function(F){if(this.index==c.p.sortname||this.name==c.p.sortname){if(c.p.lastsort==F)R=true;t=F;A=this.sorttype;return false}});M=A=="float"||A=="number"||A=="currency"?function(F){F=parseFloat(F.replace(p,""));return isNaN(F)?0:F}:A=="int"||A=="integer"?function(F){return q(F.replace(p,""),0)}:A=="date"||A=="datetime"?function(F){return sa(c.p.colModel[t].datefmt||"Y-m-d",F).getTime()}:function(F){return a.trim(F.toUpperCase())};
a.each(c.rows,function(F,L){try{B=a.unformat(a(L).children("td").eq(t),{rowId:L.id,colModel:c.p.colModel[t]},t,true)}catch(X){B=a(L).children("td").eq(t).text()}L.sortKey=M(B);s[F]=this});if(c.p.treeGrid)a(c).jqGrid("SortTree",I);else{R?s.reverse():s.sort(function(F,L){if(F.sortKey<L.sortKey)return-I;if(F.sortKey>L.sortKey)return I;return 0});if(s[0]){a("td",s[0]).each(function(F){a(this).css("width",g.headers[F].width+"px")});c.grid.cols=s[0].cells}var O="";if(c.p.altRows)O=c.p.altclass;a.each(s,
function(F,L){if(O)F%2==1?a(L).addClass(O):a(L).removeClass(O);a("tbody",c.grid.bDiv).append(L);L.sortKey=null})}c.grid.bDiv.scrollTop=0},sa=function(p,s){var t={m:1,d:1,y:1970,h:0,i:0,s:0},A,B,M;if(s=a.trim(s)){s=s.split(/[\\\/:_;.\t\T\s-]/);p=p.split(/[\\\/:_;.\t\T\s-]/);var I=a.jgrid.formatter.date.monthNames;A=0;for(B=p.length;A<B;A++){if(p[A]=="M"){M=a.inArray(s[A],I);if(M!==-1&&M<12)s[A]=M+1}if(p[A]=="F"){M=a.inArray(s[A],I);if(M!==-1&&M>11)s[A]=M+1-12}t[p[A].toLowerCase()]=parseInt(s[A],10)}t.m=
parseInt(t.m,10)-1;p=t.y;if(p>=70&&p<=99)t.y=1900+t.y;else if(p>=0&&p<=69)t.y=2E3+t.y}return new Date(t.y,t.m,t.d,t.h,t.i,t.s,0)};k=function(p,s){var t="",A="<table cellspacing='0' cellpadding='0' border='0' style='table-layout:auto;' class='ui-pg-table'><tbody><tr>",B="",M,I,R,O,F=function(L){c.p.selrow=null;if(c.p.multiselect){c.p.selarrrow=[];a("#cb_"+a.jgrid.jqID(c.p.id),c.grid.hDiv).attr("checked",false)}c.p.savedRow=[];if(a.isFunction(c.p.onPaging))if(c.p.onPaging.call(c,L)=="stop")return false;
return true};p=p.substr(1);M="pg_"+p;I=p+"_left";R=p+"_center";O=p+"_right";a("#"+p).append("<div id='"+M+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;' role='row'><tbody><tr><td id='"+I+"' align='left'></td><td id='"+R+"' align='center' style='white-space:pre;'></td><td id='"+O+"' align='right'></td></tr></tbody></table></div>").attr("dir","ltr");if(c.p.rowList.length>0){B="<td dir='"+j+"'>";B+=
...
if(!F("records"))return false;P();return false});if(c.p.pgbuttons===true){a(".ui-pg-button","#"+M).hover(function(){if(a(this).hasClass("ui-state-disabled"))this.style.cursor="default";else{a(this).addClass("ui-state-hover");this.style.cursor="pointer"}},function(){if(!a(this).hasClass("ui-state-disabled")){a(this).removeClass("ui-state-hover");this.style.cursor="default"}});a("#first"+s+", #prev"+s+", #next"+s+", #last"+s,"#"+p).click(function(){var L=q(c.p.page,1),X=q(c.p.lastpage,1),ca=false,ha=
true,pa=true,na=true,ra=true;if(X===0||X===1)ra=na=pa=ha=false;else if(X>1&&L>=1)if(L===1)pa=ha=false;else{if(!(L>1&&L<X))if(L===X)ra=na=false}else if(X>1&&L===0){ra=na=false;L=X-1}if(this.id==="first"+s&&ha){c.p.page=1;ca=true}if(this.id==="prev"+s&&pa){c.p.page=L-1;ca=true}if(this.id==="next"+s&&na){c.p.page=L+1;ca=true}if(this.id==="last"+s&&ra){c.p.page=X;ca=true}if(ca){if(!F(this.id))return false;P()}return false})}c.p.pginput===true&&a("input.ui-pg-input","#"+M).keypress(function(L){if((L.charCode?
L.charCode:L.keyCode?L.keyCode:0)==13){c.p.page=a(this).val()>0?a(this).val():c.p.page;if(!F("user"))return false;P();return false}return this})};var za=function(p,s,t,A){if(c.p.colModel[s].sortable)if(!(c.p.savedRow.length>0)){if(!t){if(c.p.lastsort==s)if(c.p.sortorder=="asc")c.p.sortorder="desc";else{if(c.p.sortorder=="desc")c.p.sortorder="asc"}else c.p.sortorder="asc";c.p.page=1}if(A)if(c.p.lastsort==s&&c.p.sortorder==A&&!t)return;else c.p.sortorder=A;t=a("thead:first",c.grid.hDiv).get(0);a("tr th:eq("+
...
repeatitems:true,cell:"cell"}},c.p.jsonReader);if(c.p.scroll){c.p.pgbuttons=false;c.p.pginput=false;c.p.rowList=[]}var G="<thead><tr class='ui-jqgrid-labels' role='rowheader'>",S,K,W,Q,T,U,J,ba;K=ba="";if(c.p.shrinkToFit===true&&c.p.forceFit===true)for(h=c.p.colModel.length-1;h>=0;h--)if(!c.p.colModel[h].hidden){c.p.colModel[h].resizable=false;break}if(c.p.viewsortcols[1]=="horizontal"){ba=" ui-i-asc";K=" ui-i-desc"}S=o?"class='ui-th-div-ie'":"";ba="<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+
ba+" ui-state-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+j+"'></span>";ba+="<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+K+" ui-state-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+j+"'></span></span>";for(h=0;h<this.p.colNames.length;h++){G+="<th role='columnheader' class='ui-state-default ui-th-column ui-th-"+j+"'>";K=c.p.colModel[h].index||c.p.colModel[h].name;G+="<div id='jqgh_"+c.p.colModel[h].name+"' "+S+">"+c.p.colNames[h];if(K==c.p.sortname)c.p.lastsort=h;G+=ba+"</div></th>"}G+=
...
G>0?G:"nw"}(function(){var p=0,s=c.p.cellLayout,t=0,A,B=c.p.scrollOffset,M,I=false,R,O=0,F=0,L=0,X;if(m)s=0;a.each(c.p.colModel,function(){if(typeof this.hidden==="undefined")this.hidden=false;if(this.hidden===false){p+=q(this.width,0);if(this.fixed){O+=this.width;F+=this.width+s}else t++;L++}});if(isNaN(c.p.width))c.p.width=g.width=p;else g.width=c.p.width;c.p.tblwidth=p;if(c.p.shrinkToFit===false&&c.p.forceFit===true)c.p.forceFit=false;if(c.p.shrinkToFit===true&&t>0){R=g.width-s*t-F;if(!isNaN(c.p.height)){R-=
B;I=true}p=0;a.each(c.p.colModel,function(ca){if(this.hidden===false&&!this.fixed){this.width=M=Math.floor(R/(c.p.tblwidth-O)*this.width);p+=M;A=ca}});X=0;if(I){if(g.width-F-(p+s*t)!==B)X=g.width-F-(p+s*t)-B}else if(!I&&Math.abs(g.width-F-(p+s*t))!==1)X=g.width-F-(p+s*t);c.p.colModel[A].width+=X;c.p.tblwidth=p+X+O+L*s}})();a(n).css("width",g.width+"px").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+c.p.id+"'>&#160;</div>");a(l).css("width",g.width+"px");G=a("thead:first",c).get(0);var wa="<table role='grid' style='width:"+
c.p.tblwidth+"px' class='ui-jqgrid-ftable' cellspacing='0' cellpadding='0' border='0'><tbody><tr role='row' class='ui-widget-content footrow footrow-"+j+"'>";l=a("tr:first",G);c.p.disableClick=false;a("th",l).each(function(p){a("div",this);W=c.p.colModel[p].width;if(typeof c.p.colModel[p].resizable==="undefined")c.p.colModel[p].resizable=true;if(c.p.colModel[p].resizable){Q=document.createElement("span");a(Q).html("&#160;").addClass("ui-jqgrid-resize ui-jqgrid-resize-"+j);!a.browser.opera&&a(Q).css("cursor",
"col-resize");a(this).addClass(c.p.resizeclass)}else Q="";a(this).css("width",W+"px").prepend(Q);c.p.colModel[p].hidden&&a(this).css("display","none");g.headers[p]={width:W,el:this};T=c.p.colModel[p].sortable;if(typeof T!=="boolean")T=c.p.colModel[p].sortable=true;var s=c.p.colModel[p].name;s=="cb"||s=="subgrid"||s=="rn"||c.p.viewsortcols[2]&&a("div",this).addClass("ui-jqgrid-sortable");if(T)if(c.p.viewsortcols[0]){a("div span.s-ico",this).show();p==c.p.lastsort&&a("div span.ui-icon-"+c.p.sortorder,
...
c.p.page=p;c.grid.bDiv.scrollTop=c.grid.prevRowHeight?(p-1)*c.grid.prevRowHeight*c.p.rowNum:0}if(c.grid.prevRowHeight&&c.p.scroll){delete c.p.lastpage;c.grid.populateVisible()}else c.grid.populate();return false});a.isFunction(this.p.ondblClickRow)&&a(this).dblclick(function(p){U=p.target;J=a(U,c.rows).closest("tr.jqgrow");if(a(J).length===0)return false;ma=J[0].rowIndex;oa=a.jgrid.getCellIndex(U);c.p.ondblClickRow.call(c,a(J).attr("id"),ma,oa,p);return false});a.isFunction(this.p.onRightClickRow)&&
a(this).bind("contextmenu",function(p){U=p.target;J=a(U,c.rows).closest("tr.jqgrow");if(a(J).length===0)return false;c.p.multiselect||a(c).jqGrid("setSelection",J[0].id,true);ma=J[0].rowIndex;oa=a.jgrid.getCellIndex(U);c.p.onRightClickRow.call(c,a(J).attr("id"),ma,oa,p);return false});g.bDiv=document.createElement("div");a(g.bDiv).append(a('<div style="position:relative;'+(o&&a.browser.version<8?"height:0.01%;":"")+'"></div>').append("<div></div>").append(this)).addClass("ui-jqgrid-bdiv").css({height:c.p.height+
...
if(E.length){var G=E[0].name;a.each(m.p.colModel,function(){if(this.name==G&&this.editoptions&&a.isFunction(this.editoptions.custom_value)){try{x[G]=this.editoptions.custom_value(a("#"+G,"#"+r),"get");if(x[G]===undefined)throw"e1";}catch(K){K=="e1"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.novalue,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,K.message,jQuery.jgrid.edit.bClose)}return true}})}else{switch(a(this).get(0).type){case "checkbox":if(a(this).attr("checked"))x[this.name]=
a(this).val();else{E=a(this).attr("offval");x[this.name]=E}break;case "select-one":x[this.name]=a("option:selected",this).val();N[this.name]=a("option:selected",this).text();break;case "select-multiple":x[this.name]=a(this).val();x[this.name]=x[this.name]?x[this.name].join(","):"";var S=[];a("option:selected",this).each(function(K,W){S[K]=a(W).text()});N[this.name]=S.join(",");break;case "password":case "text":case "textarea":case "button":x[this.name]=a(this).val();break}if(m.p.autoencode)x[this.name]=
a.jgrid.htmlEncode(x[this.name])}});return true}function c(E,G,S,K){for(var W,Q,T,U=0,J,ba,Z,ka=[],aa=false,wa,ta,la="",ma=1;ma<=K;ma++)la+="<td class='CaptionTD ui-widget-content'>&#160;</td><td class='DataTD ui-widget-content' style='white-space:pre'>&#160;</td>";if(E!="_empty")aa=a(G).jqGrid("getInd",E);a(G.p.colModel).each(function(oa){W=this.name;ba=(Q=this.editrules&&this.editrules.edithidden==true?false:this.hidden===true?true:false)?"style='display:none'":"";if(W!=="cb"&&W!=="subgrid"&&this.editable===
true&&W!=="rn"){if(aa===false)J="";else if(W==G.p.ExpandColumn&&G.p.treeGrid===true)J=a("td:eq("+oa+")",G.rows[aa]).text();else try{J=a.unformat(a("td:eq("+oa+")",G.rows[aa]),{rowId:E,colModel:this},oa)}catch(Ia){J=a("td:eq("+oa+")",G.rows[aa]).html()}var xa=a.extend({},this.editoptions||{},{id:W,name:W});frmopt=a.extend({},{elmprefix:"",elmsuffix:"",rowabove:false,rowcontent:""},this.formoptions||{});wa=parseInt(frmopt.rowpos)||U+1;ta=parseInt((parseInt(frmopt.colpos)||1)*2);if(E=="_empty"&&xa.defaultValue)J=
a.isFunction(xa.defaultValue)?xa.defaultValue():xa.defaultValue;if(!this.edittype)this.edittype="text";if(m.p.autoencode)J=a.jgrid.htmlDecode(J);Z=createEl(this.edittype,xa,J,false,a.extend({},a.jgrid.ajaxOptions,G.p.ajaxSelectOptions||{}));if(J==""&&this.edittype=="checkbox")J=a(Z).attr("offval");if(d.checkOnSubmit||d.checkOnUpdate)d._savedData[W]=J;a(Z).addClass("FormElement");T=a(S).find("tr[rowpos="+wa+"]");if(frmopt.rowabove){xa=a("<tr><td class='contentinfo' colspan='"+K*2+"'>"+frmopt.rowcontent+
"</td></tr>");a(S).append(xa);xa[0].rp=wa}if(T.length==0){T=a("<tr "+ba+" rowpos='"+wa+"'></tr>").addClass("FormData").attr("id","tr_"+W);a(T).append(la);a(S).append(T);T[0].rp=wa}a("td:eq("+(ta-2)+")",T[0]).html(typeof frmopt.label==="undefined"?G.p.colNames[oa]:frmopt.label);a("td:eq("+(ta-1)+")",T[0]).append(frmopt.elmprefix).append(Z).append(frmopt.elmsuffix);ka[U]=oa;U++}});if(U>0){ma=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(K*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='"+
G.p.id+"_id' value='"+E+"'/></td></tr>");ma[0].rp=U+999;a(S).append(ma);if(d.checkOnSubmit||d.checkOnUpdate)d._savedData.id=E}return ka}function l(E,G,S){var K,W=0,Q,T,U,J,ba;if(d.checkOnSubmit||d.checkOnUpdate){d._savedData={};d._savedData.id=E}var Z=G.p.colModel;if(E=="_empty"){a(Z).each(function(){K=this.name;U=a.extend({},this.editoptions||{});T=a("#"+a.jgrid.jqID(K),"#"+S);if(T[0]!=null){J="";if(U.defaultValue){J=a.isFunction(U.defaultValue)?U.defaultValue():U.defaultValue;if(T[0].type=="checkbox"){ba=
J.toLowerCase();if(ba.search(/(false|0|no|off|undefined)/i)<0&&ba!==""){T[0].checked=true;T[0].defaultChecked=true;T[0].value=J}else T.attr({checked:"",defaultChecked:""})}else T.val(J)}else if(T[0].type=="checkbox"){T[0].checked=false;T[0].defaultChecked=false;J=a(T).attr("offval")}else if(T[0].type.substr(0,6)=="select")T[0].selectedIndex=0;else T.val(J);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[K]=J}});a("#id_g","#"+S).val(E)}else{var ka=a(G).jqGrid("getInd",E,true);if(ka){a("td",
ka).each(function(aa){K=Z[aa].name;if(K!=="cb"&&K!=="subgrid"&&K!=="rn"&&Z[aa].editable===true){if(K==G.p.ExpandColumn&&G.p.treeGrid===true)Q=a(this).text();else try{Q=a.unformat(this,{rowId:E,colModel:Z[aa]},aa)}catch(wa){Q=a(this).html()}if(m.p.autoencode)Q=a.jgrid.htmlDecode(Q);if(d.checkOnSubmit===true||d.checkOnUpdate)d._savedData[K]=Q;K=a.jgrid.jqID(K);switch(Z[aa].edittype){case "password":case "text":case "button":case "image":a("#"+K,"#"+S).val(Q);break;case "textarea":if(Q=="&nbsp;"||Q==
"&#160;"||Q.length==1&&Q.charCodeAt(0)==160)Q="";a("#"+K,"#"+S).val(Q);break;case "select":var ta=Q.split(",");ta=a.map(ta,function(ma){return a.trim(ma)});a("#"+K+" option","#"+S).each(function(){this.selected=!Z[aa].editoptions.multiple&&(ta[0]==a.trim(a(this).text())||ta[0]==a.trim(a(this).val()))?true:Z[aa].editoptions.multiple?a.inArray(a.trim(a(this).text()),ta)>-1||a.inArray(a.trim(a(this).val()),ta)>-1?true:false:false});break;case "checkbox":Q+="";if(Z[aa].editoptions&&Z[aa].editoptions.value)if(Z[aa].editoptions.value.split(":")[0]==
Q){a("#"+K,"#"+S).attr("checked",true);a("#"+K,"#"+S).attr("defaultChecked",true)}else{a("#"+K,"#"+S).attr("checked",false);a("#"+K,"#"+S).attr("defaultChecked","")}else{Q=Q.toLowerCase();if(Q.search(/(false|0|no|off|undefined)/i)<0&&Q!==""){a("#"+K,"#"+S).attr("checked",true);a("#"+K,"#"+S).attr("defaultChecked",true)}else{a("#"+K,"#"+S).attr("checked",false);a("#"+K,"#"+S).attr("defaultChecked","")}}break;case "custom":try{if(Z[aa].editoptions&&a.isFunction(Z[aa].editoptions.custom_value))Z[aa].editoptions.custom_value(a("#"+
K,"#"+S),"set",Q);else throw"e1";}catch(la){la=="e1"?info_dialog(jQuery.jgrid.errors.errcap,"function 'custom_value' "+a.jgrid.edit.msg.nodefined,jQuery.jgrid.edit.bClose):info_dialog(jQuery.jgrid.errors.errcap,la.message,jQuery.jgrid.edit.bClose)}break}W++}});W>0&&a("#id_g","#"+r).val(E)}}}function k(){var E,G=[true,"",""],S={},K=m.p.prmNames,W,Q;if(a.isFunction(d.beforeCheckValues)){var T=d.beforeCheckValues(x,a("#"+q),x[m.p.id+"_id"]=="_empty"?K.addoper:K.editoper);if(T&&typeof T==="object")x=
T}for(var U in x){G=checkValues(x[U],U,m);if(G[0]==false)break}if(G[0]){if(a.isFunction(d.onclickSubmit))S=d.onclickSubmit(d,x)||{};if(a.isFunction(d.beforeSubmit))G=d.beforeSubmit(x,a("#"+q))}H=d.url?d.url:a(m).jqGrid("getGridParam","editurl");if(G[0])if(!H){G[0]=false;G[1]+=" "+a.jgrid.errors.nourl}if(G[0]===false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else if(!d.processing){d.processing=true;a("#sData","#"+r+"_2").addClass("ui-state-active");Q=K.oper;W=K.id;x[Q]=a.trim(x[m.p.id+
"_id"])=="_empty"?K.addoper:K.editoper;if(x[Q]!=K.addoper)x[W]=x[m.p.id+"_id"];else if(x[W]===undefined)x[W]=x[m.p.id+"_id"];delete x[m.p.id+"_id"];x=a.extend(x,d.editData,S);a.ajax(a.extend({url:H,type:d.mtype,data:a.isFunction(d.serializeEditData)?d.serializeEditData(x):x,complete:function(J,ba){if(ba!="success"){G[0]=false;G[1]=a.isFunction(d.errorTextFormat)?d.errorTextFormat(J):ba+" Status: '"+J.statusText+"'. Error code: "+J.status}else if(a.isFunction(d.afterSubmit))G=d.afterSubmit(J,x);if(G[0]===
false){a("#FormError>td","#"+r).html(G[1]);a("#FormError","#"+r).show()}else{a.each(m.p.colModel,function(){if(N[this.name]&&this.formatter&&this.formatter=="select")try{delete N[this.name]}catch(ka){}});x=a.extend(x,N);m.p.autoencode&&a.each(x,function(ka,aa){x[ka]=a.jgrid.htmlDecode(aa)});if(x[Q]==K.addoper){G[2]||(G[2]=parseInt(m.p.records)+1);x[W]=G[2];if(d.closeAfterAdd){if(d.reloadAfterSubmit)a(m).trigger("reloadGrid");else{a(m).jqGrid("addRowData",G[2],x,b.addedrow);a(m).jqGrid("setSelection",
...
a(m).jqGrid("setRowData",x[W],x);d.closeAfterEdit&&hideModal("#"+u.themodal,{gb:"#gbox_"+n,jqm:b.jqModal,onClose:d.onClose})}if(a.isFunction(d.afterComplete)){E=J;setTimeout(function(){d.afterComplete(E,x,a("#"+q));E=null},500)}}d.processing=false;if(d.checkOnSubmit||d.checkOnUpdate){a("#"+q).data("disabled",false);if(d._savedData.id!="_empty")d._savedData=x}a("#sData","#"+r+"_2").removeClass("ui-state-active");try{a(":input:visible","#"+q)[0].focus()}catch(Z){}},error:function(J,ba,Z){a("#FormError>td",
"#"+r).html(ba+" : "+Z);a("#FormError","#"+r).show();d.processing=false;a("#"+q).data("disabled",false);a("#sData","#"+r+"_2").removeClass("ui-state-active")}},a.jgrid.ajaxOptions,d.ajaxEditOptions))}}function o(E,G){var S=false,K;for(K in E)if(E[K]!=G[K]){S=true;break}return S}var m=this;if(m.grid&&e){var n=m.p.id,q="FrmGrid_"+n,r="TblGrid_"+n,u={themodal:"editmod"+n,modalhead:"edithd"+n,modalcontent:"editcnt"+n,scrollelm:q},v=a.isFunction(d.beforeShowForm)?d.beforeShowForm:false,w=a.isFunction(d.afterShowForm)?
...
E="",G=["integer","number","currency"],S=0,K=0,W,Q,T,U=1;U<=da;U++)E+=U==1?Ga:"<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";a(N.p.colModel).each(function(){ea=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false;if(!ea&&this.align==="right")if(this.formatter&&a.inArray(this.formatter,G)!==-1)S=Math.max(S,parseInt(this.width,10));else K=Math.max(K,parseInt(this.width,10))});W=S!==
0?S:K!==0?K:0;va=a(N).jqGrid("getInd",x);a(N.p.colModel).each(function(J){P=this.name;Q=false;za=(ea=this.editrules&&this.editrules.edithidden===true?false:this.hidden===true?true:false)?"style='display:none'":"";T=typeof this.viewable!="boolean"?true:this.viewable;if(P!=="cb"&&P!=="subgrid"&&P!=="rn"&&T){sa=va===false?"":P==N.p.ExpandColumn&&N.p.treeGrid===true?a("td:eq("+J+")",N.rows[va]).text():a("td:eq("+J+")",N.rows[va]).html();Q=this.align==="right"&&W!==0?true:false;a.extend({},this.editoptions||
...
"undefined"?N.p.colNames[J]:ba.label)+"</b>");a("td:eq("+(ka-1)+")",V[0]).append("<span>"+sa+"</span>").attr("id","v_"+P);Q&&a("td:eq("+(ka-1)+") span",V[0]).css({"text-align":"right",width:W+"px"});Ca[ga]=J;ga++}});if(ga>0){x=a("<tr class='FormData' style='display:none'><td class='CaptionTD'></td><td colspan='"+(da*2-1)+"' class='DataTD'><input class='FormElement' id='id_g' type='text' name='id' value='"+x+"'/></td></tr>");x[0].rp=ga+99;a(Y).append(x)}return Ca}function c(x,N){var Y,da,P=0,ea,V;
