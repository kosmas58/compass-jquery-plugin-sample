...
describe <%= controller_class_name %>Controller, "#route_for" do
  ...
  it "should map { :controller => '<%= name.pluralize %>', :action => 'edit', :id => 1 } to /<%= name.pluralize %>/1/edit" do
    route_for(:controller => "<%= name.pluralize %>", :action => "edit", :id => "1").should == "/<%= name.pluralize %>/1/edit"
  end
